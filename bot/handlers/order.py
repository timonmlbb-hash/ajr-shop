...
# Guruh ID lar
GROUP_ORDERS_ID   = int(os.getenv("GROUP_CHAT_ID", "-5194049252"))    # Buyurtmalar
...
class OrderState(StatesGroup):
    ...
    waiting_confirm = State()   # Savat tasdiqlash
...
async def handle_address(message: Message, state: FSMContext):
    ...
    # Savat + kiritilgan ma'lumotlarni ko'rsatish
...
async def edit_cart_callback(callback: CallbackQuery, state: FSMContext):
    await state.clear()
    from bot.handlers.cart import cart_inline_kb, get_cart, format_cart_text
...
async def handle_payment(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if payment_type == "card":
        ...
        await state.update_data(check_order_id=order.id, check_order_text=base_text)
        await callback.message.answer(
        ...
        await callback.message.answer(
            ...
            "Iltimos, telefoningizni yoqiq tutng.",
            parse_mode="HTML"
        )

    await callback.answer()

    # ─── Guruh + Glavniy adminga xabar ───────────────────────────────────────
        ...
        try:
            for item in cart_snapshot:
                ...
                if product and product.photo_url:
                    ...
                    await bot.send_photo(target_id, photo=product.photo_url, caption=cap, parse_mode="HTML")
...
async def receive_check(message: Message, state: FSMContext, bot: Bot):
    ...
    order_text = data.get("check_order_text", "")
    ...
    try:
        if message.photo:
            await bot.send_photo(GROUP_CHECKS_ID, photo=message.photo[-1].file_id,
                                 caption=caption, parse_mode="HTML")
        elif message.document:
            await bot.send_document(GROUP_CHECKS_ID, document=message.document.file_id,
                                    caption=caption, parse_mode="HTML")
        # Glavniy adminga ham
        if message.photo:
            await bot.send_photo(GLAVNIY_ADMIN_ID, photo=message.photo[-1].file_id,
                                 caption=caption, parse_mode="HTML")
    except Exception as e:
...
async def _status_update(callback: CallbackQuery, bot: Bot, order_id: int, new_status: str):
    ...
    if new_status == "confirmed":
        ...
        if order.payment_type and order.payment_type.value == "credit":
            msg += f"\n\n💰 To'lov: <b>{int(order.total_price):,} so'm</b>\n👇 To'lov uchun:"
            kb = InlineKeyboardMarkup(inline_keyboard=[[
                InlineKeyboardButton(text="💳 Paynet orqali to'lash", url=PAYNET_LINK)
            ]])

        # ─── Faqat shu yerda stock -1 ───────────────────────────────────────
        try:
            from database.crud import decrease_stock
            async with AsyncSessionLocal() as sess:
                for item in (order.items or []):
                    if item.size:
                        ok = await decrease_stock(sess, item.product_id, item.size, item.quantity)
                        if not ok:
                            print(f"⚠️ Stock kamaymadi: product={item.product_id} size={item.size}")
        except Exception as e:
        ...
        kb = None
        # Sharh so'rash (3 kun o'rniga hozircha darhol)
        try:
