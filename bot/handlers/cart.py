...
async def add_to_cart_direct(callback: CallbackQuery, product_id: int,
    ...
    async with AsyncSessionLocal() as session:
        ...
        # Session ichida hamma kerakli ma'lumotlarni olamiz
        cat_id    = product.category_id
        prod_name = product.name
        fin_price = product.price * (1 - product.discount_percent / 100) \
                    if product.discount_percent > 0 else product.price

    cart = get_cart(user_id)

    # Ism yozish xizmati (cat_id=4)
    if cat_id == 4:
    ...
    price = fin_price + (50000 if back_print else 0)
    ...
    for item in cart:
        if item["product_id"] == product_id and item.get("size") == size:
            ...
            await callback.answer(f"✅ {prod_name} miqdori oshirildi!")
            return
    ...
    await callback.answer(f"✅ {prod_name}{size_text}{print_text} savatga qo'shildi!")
    try:
        await callback.message.edit_reply_markup(reply_markup=None)
    except:
        pass
...
async def handle_print_name(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"👕 {prod_name}{f' ({size})' if size else ''}\n"
        f"✍️ Orqa: <b>{name_input}</b>\n"
        f"💰 {int(price):,} so'm (ism yozish +50,000 so'm)\n\n"
        f"🛒 <b>Savatim</b> tugmasini bosing.",
        parse_mode="HTML"
    )

    if mode == "buynow":
        # Tezkor buyurtma — ism so'rashga o'tish
...
async def handle_player_name(message: Message, state: FSMContext):
    ...
    if len(name_input) < 2 or len(name_input) > 20:
        await message.answer("⚠️ Ism 2-20 ta belgi bo'lishi kerak:")
        return
