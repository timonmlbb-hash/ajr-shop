...
# ─── In-memory cart ───────────────────────────────────────────────────────────
# {user_id: [ {product_id, name, price, qty, size, player_name, needs_name} ]}
_carts: dict = {}
...
async def add_to_cart_direct(callback: CallbackQuery, product_id: int,
    ...
    await callback.answer(f"✅ {product.name}{size_text} savatga qo'shildi!")
    try:
        await callback.message.edit_reply_markup(reply_markup=None)
    except:
        pass
...
async def handle_player_name(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "<i>Raqam kerak bo'lmasa — (tire belgisi) yuboring</i>",
        parse_mode="HTML"
    )
...
async def handle_jersey_number(message: Message, state: FSMContext):
    ...
    if num_input == "-":
        ...
        try:
            num = int(num_input)
            if not 1 <= num <= 99:
                await message.answer("⚠️ Raqam 1-99 oralig'ida bo'lishi kerak (yoki — yuboring):")
                return
    ...
    await message.answer(
        ...
        f"💰 {int(price):,} so'm\n\n"
        f"Davom etish uchun <b>🛒 Savatim</b>ni bosing.",
        parse_mode="HTML"
    )
...
async def checkout(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        "📍 <b>Yetkazish manzilini yozing:</b>\n\n"
        "<i>Masalan: Toshkent, Yunusobod, 7-mavze, 12-uy</i>",
        parse_mode="HTML",
