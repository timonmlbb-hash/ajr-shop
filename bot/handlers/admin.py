...
async def manage_admins(message: Message):
    if not is_admin(message.from_user.id):
        return
    from bot.middlewares.admin_check import ADMIN_IDS
...
async def add_admin_cmd(message: Message):
    ...
    await message.answer(
        ...
        f"⚠️ Bu faqat joriy sessiya uchun. Doimiy qilish uchun Railway Variables ga qo'shing:\n"
...
async def remove_admin_cmd(message: Message):
    ...
    await message.answer(
        f"✅ <code>{rem_id}</code> adminlikdan o'chirildi!\n\n"
